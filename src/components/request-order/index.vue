<template>
  <DashboardLayout>
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
      <h1 class="h2">Request Order</h1>
      <div class="btn-toolbar mb-2 mb-md-0">
        <div class="btn-group me-2">
          <router-link to="/request-order/create" class="btn btn-sm btn-primary">
    ...
    <div v-if="!loading && !error" class="card">
      <div class="card-header">
        <h5 class="card-title mb-0">Request Orders List</h5>
      </div>
      <div class="card-body">
        <DataTable
          :data="requestOrders"
...
export default {
    ...
    return {
      ...
      tableOptions: {
        ...
        language: {
          ...
          emptyTable: "No request orders found",
  ...
  methods: {
      ...
      try {
        ...
        if (response.data) {
          if (Array.isArray(response.data)) {
            ...
            this.requestOrders = response.data.data;
          } else if (response.data.result && Array.isArray(response.data.result)) {
            this.requestOrders = response.data.result;
          } else {
            this.requestOrders = [];
        ...
        console.log('Processed request orders:', this.requestOrders);
        
      } catch (error) {
        console.error('Error fetching request orders:', error);
        this.error = 'Failed to load request orders. Please try again later.';
    ...
    handleView(id) {
      this.$router.push({ name: 'RequestOrderView', params: { id } });
    },
    
    handleEdit(id) {
      this.$router.push({ name: 'RequestOrderEdit', params: { id } });
    },
    
    async handleDelete(id) {
      if (confirm('Are you sure you want to delete this request order?')) {
        try {
          console.log('Delete request order:', id);
          this.$notify({
            title: 'Success',
            text: 'Request order deleted successfully',
            type: 'success'
          });
          ...
          this.$notify({
            title: 'Error',
            text: 'Failed to delete request order',
            type: 'error'
          });
        }
      }
